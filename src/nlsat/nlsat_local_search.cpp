...
struct ls_helper::imp {
    ...
    imp(solver& s, anum_manager& am, pmanager& pm, polynomial::cache& cache, interval_set_manager& ism, evaluator& ev, assignment& ass, svector<lbool>& bvalues, clause_vector const& cls, atom_vector const& ats, bool_var_vector const& pure_bool_vars, bool_var_vector const& pure_bool_convert, unsigned seed, unsigned& step, unsigned& stuck, double& ratio, substitute_value_vector const& vec)
        : m_am(am), m_pm(pm), m_ism(ism), m_evaluator(ev), m_assignment(ass), m_clauses(cls), m_atoms(ats), m_rand_seed(seed), m_solver(s), m_cutoff(1200), is_bool_search(false), is_random_walk(false), m_restart_count(100), m_nra_operation_table(m_am, m_nra_operation_index, m_nra_operation_value), m_step(step), m_stuck(stuck), m_stuck_ratio(ratio), m_cache(cache), m_sub_value(vec), m_time_label(1), m_pure_bool_vars(pure_bool_vars), m_pure_bool_convert(pure_bool_convert), m_bvalues(bvalues) {
        set_const_anum();
        clear_statistics();
    }
    ...
    */
    int pick_random_move(bool_var& bvar, var& avar, anum& best_value, int& best_score, clause_index & c_idx) {
        ...
        SASSERT(!m_unsat_clauses.empty());
        // Random walk case
        c_idx = m_unsat_clauses[rand_int() % m_unsat_clauses.size()];
        nra_clause const* curr_clause = m_nra_clauses[c_idx];
        vector<var> best_arith_index;
        anum_vector arith_vals;
    ...
    void no_operation_random_walk(clause_index c_idx) {
        m_stuck++;
        m_stuck_ratio = m_stuck * 1.0 / m_step;
        SASSERT(!m_unsat_clauses.empty());
    ...
    lbool solve() {
        ...
        for (m_step = 1; m_step < max_step; m_step++) {
            ...
            int best_score;
            int mode = pick_greedy_move(picked_b, picked_v, next_value, best_score);
            ...
            if (mode == 0) {  // bool operation
                ...
                paws_weight();
                clause_index c_idx;
                int random_mode = pick_random_move(picked_b, picked_v, next_value, best_score, c_idx);
                if(random_mode == 0) {
                    critical_bool_move(picked_b);
                    if (update_bool_info()) {
                        no_improve_cnt_bool = 0;
                    ...
                    no_operation_random_walk(c_idx);
                }
            }
