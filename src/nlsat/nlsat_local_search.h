...
struct substitute_value {
    ...
    poly const* p;
    poly const* q;
    substitute_value(var v, poly const* pp, poly const* qq)
        : m_var(v), p(pp), q(qq) {}
};
...
class nra_literal {
   ...
   public:
    ...
    nra_literal(unsigned idx, unsigned b_idx, const literal l, bool is_bool, var_table const& vars, atom const* at)
        : m_index(idx), m_bool_index(b_idx), m_literal(l), m_is_bool(is_bool), m_vars(vars), m_atom(to_ineq_atom(at)), m_is_sat(false), m_activity(0) {}
...
class nra_clause {
   ...
   public:
    ...
    nra_clause(unsigned idx, clause const* cls, nra_literal_vector const& vec, var_table const& vars, var_table const& vars2)
        : m_index(idx), m_clause(cls), m_weight(1), m_vars(vars), m_is_sat(false), m_bool_var(vars2), m_critical_index(null_var), m_left_vars(vars.size()) {
        m_literals.reset();
...
struct arith_literal_activity {
    ...
    arith_literal_activity(nra_literal_vector const& vec)
        : m_nra_literals(vec) {}
