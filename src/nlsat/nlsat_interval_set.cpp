...
interval_set* interval_set_manager::mk(bool lower_open, bool lower_inf, anum const& lower, bool upper_open, bool upper_inf, anum const& upper, literal justification, clause const* cls) {
...
inline void push_back(anum_manager& am, interval_buffer& buf, bool lower_open, bool lower_inf, anum const& lower, bool upper_open, bool upper_inf, anum const& upper, literal justification) {
...
void interval_set_manager::peek_in_complement_heuristic(interval_set const* s, anum_vector& vec) {
    ...
    TRACE(
        "nlsat_ls", tout << "show insertion sample values:\n";
        for (auto ele
             : vec) {
            m_am.display(tout, ele);
            tout << " ";
        } tout
        << std::endl;);
}
