...
namespace nlsat {
    ...
    struct explain::imp {
        ...
        imp(solver & s, assignment const & x2v, polynomial::cache & u, atom_vector const & atoms, atom_vector const & x2eq, evaluator & ev):
            m_solver(s),
            m_assignment(x2v),
            m_atoms(atoms),
            m_x2eq(x2eq),
        ...
        */
        literal normalize(literal l, var max) {
            ...
            if (m_atoms[b]->is_ineq_atom()) {
                ...
                // wzh dynamic
                var max_stage = m_solver.find_var_stage(max);
                // hzw dynamic
                for (unsigned i = 0; i < sz; i++) {
                    p = a->p(i);
        ...
        */
        void keep_p_x(polynomial_ref_vector & ps, var x, polynomial_ref_vector & qs) {
            ...
            for (unsigned i = 0; i < sz; i++) {
                poly * q = ps.get(i);
                // if (max_var(q) != x) {
                if(m_solver.max_stage_poly(q) != m_solver.find_var_stage(x)){
                    qs.push_back(q);
                }
                else {
                    ps.set(j, q);
        ...
        */
        void add_cell_lits(polynomial_ref_vector & ps, var y) {
            ...
            for (unsigned k = 0; k < sz; k++) {
                ...
                // wzh dynamic
                if(m_solver.max_stage_poly(p) != m_solver.find_var_stage(y)){
                    continue;
                }
                // hzw dynamic
        ...
        */
        ...
        void simplify(literal l, eq_info & info, var max, scoped_literal & new_lit) {
            ...
            if (modified_lit) {
                ...
                // TRACE("nlsat_explain", tout << "[debug] max stage literal, max stage: " << find_var_stage(max) << std::endl;);
                ...
                if (m_solver.max_stage_literal(new_lit) < m_solver.find_var_stage(max)) {
                ...
                for (unsigned j = 0; j < num_factors; j++) {
                    ...
                    for (var y : xs) {
                        // if (y >= max)
                            // continue;
                        // wzh dynamic
                        var curr_stage = m_solver.find_var_stage(y);
        ...
        */
        ...
        void project(var x, unsigned num, literal const * ls, scoped_literal_vector & result) {
            ...
            if (!m_ps.empty()) {                
                ...
                if (x != mx_var) {
                    for (var i = 0; i < m_solver.num_arith_vars(); ++i) {
        ...
        void signed_project(polynomial_ref_vector& ps, var x) {
            ...
            for (unsigned i = 0; i < ps.size(); ++i) {
                ...
                int s = sign(p);
                // if (max_var(p) != x) {
                if(m_solver.max_stage_poly(p) != m_solver.find_var_stage(x)){
