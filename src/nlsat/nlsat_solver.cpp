...
/**
 * ---------------------------------------------------------------------------------------------------------------
 * @name CHDnlsat: Clause Level Hybrid Dynamic Nonlinear Arithmetic Satisfiability Procedure
 ...
 * 
 * @date 2023/09/11
 * @brief Question about unit propagate and real_propagate_using_clauses when resolving
 ...
 * 
 * @date 2023/10/17
 * @brief Major Revision: remove semantics decision in trail, record reason clauses for propagated literal
 *        When we resolve lazy propagation, decide literals from clauses one by one
 * 
 * @date 2023/10/17
...
namespace nlsat {
    ...
    struct solver::imp {
        ...
        unsigned_vector        m_blevels;       // bool_var -> level
        ...
        struct trail {
            enum kind { BVAR_ASSIGNMENT, ARITH_ASSIGNMENT, INFEASIBLE_UPDT, UPDT_EQ, BRANCH };
            kind               m_kind;
            hybrid_var         m_x, m_x2;
        ...
        unsigned               m_levels; // total levels during search

        // vars
        unsigned               m_num_hybrid_vars;
        bool_var_vector        m_pure_bool_vars; // pure bool index --> atom index
        bool_var_vector        m_pure_bool_convert; // atom index --> pure bool index
        ...
        // watchers
        vector<vector<atom_var_watcher *>>       m_var_watching_atoms; // var -> atom
        vector<vector<clause_var_watcher *>>     m_var_watching_clauses, m_var_watching_learned; // var -> clause, learned
        vector<vector<clause_literal_watcher *>> m_pos_literal_watching_clauses, m_neg_literal_watching_clauses; // literal -> clause
        vector<vector<clause_literal_watcher *>> m_pos_literal_watching_learned, m_neg_literal_watching_learned; // literal -> learned
        
        // unit atom (static)
        vector<unsigned_vector>                  m_static_unit_atom; // var -> static unit atom index

        // we do not maintain static unit clause, since it is recorded with method of frontend infeasible set
        // currently we maintain static unit learned clause, because we need its infeasible set right away during search
        vector<unsigned_vector>                  m_arith_static_unit_learned; // var -> static unit learned index

        // unit atoms and clauses (dynamical)
        unsigned_vector                          m_atom_unit_clauses, m_atom_unit_learned; // atom -> unit clause, learned
        vector<unsigned_vector>                  m_arith_unit_atom; // var -> arith atom
        vector<unsigned_vector>                  m_arith_unit_clauses, m_arith_unit_clauses_more_lits; // var -> clause
        vector<unsigned_vector>                  m_arith_unit_learned, m_arith_unit_learned_more_lits; // var -> learned

        // new added unit clause or learned this propagation turn
        ...
        // restart
        unsigned               m_restart_conflicts; // conflict since this restart
        unsigned               m_restart_threshold;  // threshold (conflict) of restart
        unsigned               m_restarts; // restart times
        unsigned               m_learned_added; // count of generated learned
        unsigned               m_learned_deleted; // count of deleted learned

        // reduce
        double                 clause_bump = 1;
        double                 max_learnts;
        double                 m_reduce_threshold; // threshold (conflict) of reduce
        unsigned               m_reduces; // reduce times
        unsigned               m_reduce_conflicts; // conflict since this reduce
        int                    learntsize_adjust_cnt;
        int                    learntsize_adjust_confl;

        // valued: atom assigned or its arith vars are all assigned
        var_table              m_valued_atom_table; // hashset
        var_vector             m_valued_atom_trail; // linear DS for valued atoms
        unsigned_vector        m_valued_atom_indices; // assigned atom order
        unsigned               m_atom_prop; // count of prop using atom (unit prop)
        
        // infeasible set
        var_vector             m_infeasible_var_trail; // linear DS for infeasible set updated vars
        unsigned               m_infeasible_prop; // count of prop using infeasible updt

        // vars assignment trail
        var_vector             m_bool_trail, m_arith_trail, m_hybrid_trail; // linear DS for assigned vars
        unsigned_vector        m_hybrid_assigned_indices; // assigned order
        unsigned               m_arith_atom_prop; // count of prop using arith var (prop atom)
        unsigned               m_hybrid_var_clause_prop, m_hybrid_var_learned_prop; // count of prop using hybrid var (prop clause, learned)

        // var --> atom, clause, learned
        vector<var_vector>     m_var_atoms, m_var_clauses, m_var_learned;
        vector<var_vector>     m_bool_clauses, m_bool_learned;

        using interval_set_bool = std::pair<bool, interval_set*>;
        // Cache for infeasible set computation
        vector<vector<interval_set_bool>>  m_var_atom_infeasible_set, m_var_clause_infeasible_set, m_var_learned_infeasible_set;

        // level management
        hybrid_var             m_hk;
        unsigned               m_scope_lvl;
        unsigned_vector        m_hybrid_find_level;
        
        // conflict
        clause *               m_conflict_clause;
        ...
        bool_var mk_bool_var_core() {
            ...
            m_blevels       .setx(b, UINT_MAX, UINT_MAX);
        ...
        */
        ...
        //   - sort by max var
        //   - sort literal with the max degree
        ...
        struct lit_lt {
            ...
            bool operator()(literal l1, literal l2) const {
                ...
                if (a2 == nullptr)
                    return false;
                var v1 = m.max_level_atom(l1.var()), v2 = m.max_level_atom(l2.var());
                if(v1 < v2) {
                    return true;
                }
                if(v1 > v2) {
                ...
                var x1 = m.max_degree(a1);
                var x2 = m.max_degree(a2);
                if (x1 < x2)
                    return true;
                if (x1 > x2)
                    return false;
        ...
        // remove literals which can be propagated false using frontend information
        void simplify_learned(clause &cls, bool &deleted) {
            deleted = false;
            unsigned j = 0;
            for(literal l: cls) {
                if(frontend_value(l) == l_false) {
                    continue;
                } else if(frontend_value(l) == l_true) {
                    deleted = true;
                    del_clause(&cls);
                    return;
                } else if(is_arith_literal(l) && m_nlsat_atoms[l.var()]->m_vars.size() == 1) {
                    ...
                    if (m_ism.subset(curr_st, m_frontend_infeasible[v])) {
                        deleted = true;
                        del_clause(&cls);
                        return;
                    } else if (m_ism.is_union_full(curr_st, m_frontend_infeasible[v])) {
                        continue;
                    }
                }
                cls[j++] = l;
        ...
        clause * mk_clause_core(unsigned num_lits, literal const * lits, bool learned, _assumption_set a, bool &deleted) {
            ...
            CDTRACE(deleted, std::cout << "[DELETED] clause is deleted" << std::endl;);
            return !deleted ? cls : nullptr;
        }

        clause * mk_clause(unsigned num_lits, literal const * lits, bool learned, _assumption_set a) {
            SASSERT(num_lits > 0);
            bool deleted;
            clause * cls = mk_clause_core(num_lits, lits, learned, a, deleted);
            ++m_lemma_count;
            if(!deleted) {
        ...
        // remove literals whose index is contained in vec (falsed literals)
        void simplify_clause(clause &cls, bool_var_vector const& vec, bool &deleted) {
            deleted = false;
            unsigned j = 0;
            for(unsigned i = 0; i < cls.size(); i++) {
                literal l = cls[i];
                if(!vec.contains(l.var())) {
                    cls[j++] = l;
                }
            }
            if(j == 0) {
            ...
            if(m_vars.size() == 1) { // unit clause
                ...
                if(hybrid_is_bool(v)) { 
                    ...
                    if(!m_assignment.is_assigned(av)) {
                        ...
                        m_newly_unit_learned.push_back(std::make_pair(av, idx));
                ...
                if(hybrid_var_assigned(v1) && !hybrid_var_assigned(v2)) { // unit to v2
                    ...
                    if(hybrid_is_arith(v1)) {
                        ...
                        if (!update_learned_infeasible_set(idx, av)) {
                            m_conflict_clause = m_learned[idx];
                        }
                    }
                }
            }
        ...
        void get_learned_last_assigned_two_literal_indices(unsigned idx, unsigned &id1, unsigned &id2) {
            ...
            for(int i = 0; i < m_learned[idx]->size(); i++) {
                ...
                unsigned index = m_valued_atom_indices[l.var()];
                if(index == null_var) {
                    continue;
                }
                if(max1 == null_var) {
        ...
        void set_learned_literal_watcher(unsigned idx) {
            ...
            if(curr_clause.size() == 1) { // unit clause
                ...
                if(id1 == null_var && id2 == null_var) { // conflict
                    m_conflict_clause = is_sat ? nullptr : m_learned[idx];
        ...
        */
        ...
        void register_valued_atom(bool_var b) {
            if(!m_valued_atom_table.contains(b)) {
                ...
                m_valued_atom_indices[b] = m_valued_atom_trail.size();
        ...
        void erase_valued_atom(bool_var b) {
            if(m_valued_atom_table.contains(b)) {
                ...
                m_valued_atom_indices[b] = null_var;
            }
        }
            ...
            for(int i = 0; i < m_var_watching_clauses[x].size(); i++) {
                ...
                if(another_var == null_var || is_hybrid_assigned(another_var)) { // previously all assigned
                    if(hybrid_is_arith(x)) {
                        ...
                        m_newly_unit_clauses.push_back(std::make_pair(v, idx));
                    ...
                    if(hybrid_is_arith(another_var)) {
                        ...
                        m_newly_unit_clauses.erase(std::make_pair(v, idx));
                    }
                }
            }
        }
        ...
        void update_unit_learned_after_var_unassigned(hybrid_var x) {
            if(hybrid_is_arith(x)) {
                ...
                for(unsigned idx: m_arith_static_unit_learned[v]) {
                    ...
                    m_newly_unit_learned.push_back(std::make_pair(v, idx));
            ...
            for(int i = 0; i < m_var_watching_learned[x].size(); i++) {
                ...
                if(another_var == null_var || is_hybrid_assigned(another_var)) { // previously all assigned
                    ...
                    if(hybrid_is_arith(another_var)) {
                        ...
                        m_newly_unit_learned.erase(std::make_pair(v, idx));
        ...
        */
        ...
        struct empty_arith_pred {
            assignment const& ass;
            empty_arith_pred(assignment const& a): ass(a) {}
            bool operator()() const {
                return !ass.empty();
            }
        };

        void undo_until_empty_arith_assignment() {
            undo_until(empty_arith_pred(m_assignment));
        }
        ...
        */
        ...
        void assign_literal(literal l, justification j) {
            ...
            m_blevels[b]  = m_scope_lvl;
            m_justifications[b] = j;
            save_bool_assign_trail(b);
            updt_eq(b, j);
            m_valued_atom_indices[l.var()] = m_valued_atom_trail.size();
        ...
        /**
           \brief assign l to true, clause set propagates literal to be true
           \example c1: x >= y \/ x >= -1, when y -> 0, c1 propagates x1 >= -2 is true
        */
        clause_vector clauses;
        void real_propagate_using_clauses(literal l, interval_set const * s) {
            m_ism.get_clauses(s, clauses);
            assign_literal(l, mk_clause_lazy_jst(l, clauses));
        ...
        void updt_eq(bool_var b, justification j) {
            ...
            switch (j.get_kind()) {
                case justification::CLAUSE:
                    ...
                    break;            
                case justification::CLAUSE_LAZY:
                    return;
                default:
                    break;
            }
        ...
        */
        clause* real_propagate_clauses() {
            ...
            for(auto ele: m_newly_unit_clauses) {
                var v = ele.first; unsigned idx = ele.second;
                if(!update_clause_infeasible_set(idx, v)) {
        ...
        */
        clause* real_propagate_learned() {
            ...
            for(auto ele: m_newly_unit_learned) {
                var v = ele.first; unsigned idx = ele.second;
                if(!update_learned_infeasible_set(idx, v)) {
        ...
        */
        ...
        bool check_propagate_counter() const {
            return m_atom_prop < m_valued_atom_trail.size() || !m_newly_unit_clauses.empty() || !m_newly_unit_learned.empty();
        }
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (is_hybrid_assigned(another_var)) { // clause is all assigned
                        ...
                        if (!is_sat) { // conflict clause, this situation should be found when updating the last unassigned var
                            ...
                            // b1 \/ b2, b1 \/ !b2
                            m_conflict_clause = m_clauses[idx];
                            return;
                        }
                            ...
                            if(!found_replace) { // unit to another var
                                ...
                                if(hybrid_is_arith(another_var)) { // arith var
                                    ...
                                    if(check_learned_unit(idx, arith_var)) {
                                        ...
                                        m_newly_unit_learned.push_back(std::make_pair(arith_var, idx));
                                        if(m_learned[idx]->size() > 1) {
        ...
        */
        lbool solve() {
            ...
            while(true) {
                ...
                if(m_conflict_clause == nullptr) { // not found conflict clause when registering learned clauses
                    if((m_conflict_clause = overall_propagate()) == nullptr) { // consistent
                    ...
                    else if(!resolve(*m_conflict_clause)) return l_false;
                }
        ...
        void update_learnt_indices() {
            ...
            int j = 0;
            for(unsigned i = 0; i < m_newly_unit_learned.size(); i++) {
                unsigned origin_idx = m_newly_unit_learned[i].second;
                var v = m_newly_unit_learned[i].first;
                if(m_learned_indices[origin_idx] == null_var) {
                    continue;
                } else {
                    m_newly_unit_learned[j++] = std::make_pair(v, m_learned_indices[origin_idx]);
                }
            }
            m_newly_unit_learned.shrink(j);
        ...
        void register_nlsat_avar(var x) {
            ...
            m_hybrid_find_level.setx(arith2hybrid(x), null_var, null_var);
        ...
        void register_nlsat_bvar(bool_var b) {
            ...
            m_num_hybrid_vars++;
            m_valued_atom_indices.setx(b, null_var, null_var);
            m_hybrid_assigned_indices.setx(b, null_var, null_var);
            m_hybrid_find_level.setx(b, null_var, null_var);
         ...
         void register_nlsat_atom(bool_var b) {
            m_nlsat_atoms.enlarge(b, nullptr);
            m_valued_atom_indices.enlarge(b, null_var);
            var_table vars;
        ...
        void register_nlsat_learned(unsigned idx) {
            DTRACE(std::cout << "register learned" << std::endl;);
        ...
        void del_nlsat_atom(bool_var b) {
            ...
            if(m_valued_atom_table.contains(b)) { // previously assigned
                ...
                if(m_atom_prop > m_valued_atom_indices[b]) {
                    m_atom_prop--;
                }
                m_valued_atom_indices[b] = null_var;
            }
        ...
        void del_nlsat_clause(unsigned id) {
            ...
            for(var v: m_nlsat_clauses[id]->m_vars) { // change var's unit clause
                ...
                j = 0;
                for(unsigned i = 0; i < m_newly_unit_clauses.size(); i++) {
                    if(m_newly_unit_clauses[i].second != id) {
                        m_newly_unit_clauses[j++] = m_newly_unit_clauses[i];
                    }
                }
                m_newly_unit_clauses.shrink(j);
                j = 0;
        ...
        void del_nlsat_learned(unsigned id) {
            ...
            int jj = 0;
            for(unsigned i = 0; i < m_newly_unit_learned.size(); i++) {
                var v = m_newly_unit_learned[i].first; unsigned curr_id = m_newly_unit_learned[i].second;
                if(curr_id != id) {
                    m_newly_unit_learned[jj++] = m_newly_unit_learned[i];
                }
            }
            m_newly_unit_learned.shrink(jj);
        ...
        void set_atom_arith_watcher(unsigned idx, bool &deleted) {
            deleted = false;
        ...
        // fix bug: even if a var and another var don't share an atom, we delete cache
        void update_infeasible_cache_using_arith(var v) {
            for(unsigned idx: m_var_atoms[v]) {
                for(var nv: m_nlsat_atoms[idx]->m_vars) {
                    if(nv != v) {
                        m_var_atom_infeasible_set[nv][idx].first = false;
                    }
                }
            }
            for(unsigned idx: m_var_clauses[v]) {
                for(var nv: m_nlsat_clauses[idx]->m_vars) {
                    if(nv != v) {
            ...
            for(unsigned idx: m_var_learned[v]) {
                for(var nv: m_nlsat_learned[idx]->m_arith_vars) {
                    if(nv != v) {
        ...
        interval_set* get_atom_infeasible_set(bool_var idx, var v, bool sign, clause *cls) {
        ...
        interval_set* get_learned_infeasible_set(int idx, var v) {
            if(m_var_learned_infeasible_set[v][idx].first) { // cached before
                ...
                for(literal l: *m_learned[idx]) {
                    if(m_nlsat_atoms[l.var()]->m_vars.contains(v)) {
                        atom_st = get_atom_infeasible_set(l.var(), v, l.sign(), m_learned[idx]);
                        curr_st = m_ism.mk_intersection(curr_st, atom_st);
                    } else {
                        lbool v = value(l);
                        SASSERT(v != l_undef);
        ...
        void set_clause_literal_watcher(unsigned idx, bool & deleted) {
            ...
            simplify_clause(curr_clause, falsed_atoms, deleted);
            if(deleted) { // all literals are falsed atoms
                frontend_conflict = true;
            } else if(curr_clause.size() == 1) {
                literal l = curr_clause[0];
        ...
        lbool check() {
            ...
            if (frontend_conflict) return l_false; // found conflict in frontend
            init_solver();
            return search_check();
        }
        ...
        void init_solver() {
            ...
            if(OPTIONS::enable_restart) init_restart_options();
        }
        ...
        void build_frontend_assignment() {
            for(bool_var b = 0; b < m_atoms.size(); b++) {
                if(frontend_value(b) != l_undef && m_frontend_unit_indices[b] != null_var && m_frontend_used[b] && !m_dead[b]) {
                    assign_literal(
        ...
        void resolve_clause_lazy_justification(bool_var b, clause_lazy_justification const & jst) {
            ...
            DTRACE(
                std::cout << "new valid clause in explain:\n";
       ...
       unsigned find_hybrid_var_level(hybrid_var x) const {
            return x < m_hybrid_find_level.size() ? m_hybrid_find_level[x] : null_var;
       }

        bool same_level_literal(literal l, unsigned lvl) const {
            return max_level_literal(l) == lvl;
        }

        unsigned max_level_literal(literal l) const {
            return max_level_atom(l.var());
        }

        bool same_level_atom(bool_var b, unsigned lvl) const {
            return max_level_atom(b) == lvl;
        }

        unsigned max_degree(atom const * a) const {
            if(a == nullptr) {
                return 0;
            }
            var_vector vars;
            ...
            if(a->is_ineq_atom()) {
                ...
                poly *p = ia->p(0);
                for(unsigned i = 1; i < ia->size(); i++) {
                    p = m_pm.mul(p, ia->p(i));
                }
                m_pm.vars(p, vars);
                for(var v: vars) {
                    unsigned d = m_pm.degree(p, v);
                    if(d > res) {
                        res = d;
                    }
                }
            } else {
                root_atom const *ra = to_root_atom(a);
                m_pm.vars(ra->p(), vars);
                poly *p = ra->p();
                for(var v: vars) {
                    unsigned d = m_pm.degree(p, v);
        ...
        unsigned max_level_atom(bool_var b) const {
            if(m_atoms[b] == nullptr) {
                return find_hybrid_var_level(m_pure_bool_convert[b]);
            } else {
                if(b >= m_nlsat_atoms.size()) { // not registered yet, before the main search
                    return null_var;
                }
                nlsat_atom const *a = m_nlsat_atoms[b];
                unsigned res = 0;
                for(var v: a->m_vars) {
                    unsigned curr_stage = find_hybrid_var_level(arith2hybrid(v));
        ...
        unsigned max_level_poly(poly const *p) const {
            var_vector curr;
            m_pm.vars(p, curr);
            unsigned x = 0;
            for(var v: curr) {
                unsigned curr_stg = find_hybrid_var_level(arith2hybrid(v));
                if(x == 0 || curr_stg > x) {
                    x = curr_stg;
                }
                if(x == null_var) {
        ...
        var max_level_var_poly(poly const * p) const {
            var_vector curr;
            m_pm.vars(p, curr);
            var res_x = 0, max_stage = 0;
            for(var v: curr){
                var curr_stage = find_hybrid_var_level(arith2hybrid(v));
        ...
        var max_level_or_unassigned_ps(polynomial_ref_vector const & ps) const {
            var_table curr_vars;
            ...
            for(var v: curr_vars){
                ...
                var curr = find_hybrid_var_level(arith2hybrid(v));
        ...
        var max_level_or_unassigned_atom(atom const * a) const {
            var max_stage = 0, res_x = null_var;
            for(var v: m_nlsat_atoms[a->bvar()]->m_vars){
                if(!m_assignment.is_assigned(v)){
                    return v;
        ...
        */
        bool only_literals_from_previous_levels(unsigned num, literal const * ls) {
            for (unsigned i = 0; i < num; i++) {
                if(same_level_literal(ls[i], m_scope_lvl)){
                    return false;
                }
            }
            return true;
        ...
        */
        ...
        bool resolve(clause & conflict) {
            ...
            display(std::cout, *m_conflict_clause) << "\n";);
            ...
            resolve_clause(null_bool_var, *m_conflict_clause);
                    ...
                    if (t.m_kind == trail::BVAR_ASSIGNMENT) {
                        ...
                        if (is_marked(b)) {
                            ...
                            case justification::CLAUSE_LAZY:
                                ...
                                resolve_clause_lazy_justification(b, *(jst.get_lazy()));
                                DTRACE(std::cout << "resolve lazy done" << std::endl;);
                                break;
            ...
            m_conflict_clause = nullptr; // clear conflict this time
        ...
        void collect_statistics(statistics & st) {
            ...
            st.update("nlsat total levels", m_levels);
        }
        ...
        */
        void reorder(unsigned sz, var const * p) {
            ...
            undo_until_empty_arith_assignment();
            m_cache.reset();               
            DEBUG_CODE({
                for (var x = 0; x < num_arith_vars(); x++) {
        ...
        */
        ...
        std::ostream& display(std::ostream& out, justification const& j) const {
            ...
            case justification::CLAUSE:
                ...
                break;
            case justification::CLAUSE_LAZY: {
                clause_lazy_justification const& lz = *j.get_lazy();
        ...
        std::ostream & display_var_stage(std::ostream & out) const {
            ...
            for(hybrid_var v = 0; v < m_hybrid_find_level.size(); v++) {
                out << "hybrid var: " << v << " -> " << m_hybrid_find_level[v] << std::endl;
            }
            return out;
        }
    };
    ...
    var solver::max_level_var_poly(poly const* p) const {
        return m_imp->max_level_var_poly(p);
    }

    unsigned solver::max_level_poly(poly const* p) const {
        return m_imp->max_level_poly(p);
    }

    unsigned solver::max_level_literal(literal l) const {
        return m_imp->max_level_literal(l);
    }

    unsigned solver::find_hybrid_var_level(hybrid_var x) const {
        return m_imp->find_hybrid_var_level(x);
    }

    var solver::max_level_or_unassigned_ps(polynomial_ref_vector const& ps) const {
        return m_imp->max_level_or_unassigned_ps(ps);
    }

    var solver::max_level_or_unassigned_literals(unsigned num, literal const* ls) const {
        return m_imp->max_level_or_unassigned_literals(num, ls);
    }

    var solver::max_level_or_unassigned_atom(atom const* a) const {
        return m_imp->max_level_or_unassigned_atom(a);
    }
    // dnlsat
};