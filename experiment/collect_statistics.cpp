...
void write_ins_statistics() {
    ...
    outFile << "name, result, bool var, arith var, conflict, decision, stage, step\n";
    for(auto curr_instance: instance_vec) {
        ...
        string line;
        unsigned idx = 0;
        string curr_result, curr_bool, curr_arith, curr_conf, curr_dec, curr_stg, curr_added, curr_step;
        while(getline(inFile, line)) {
            if(idx == 0) {
                curr_result = line;
            }
            ...
            else if(line.find(":nlsat-local-search-step") != string::npos) {
                curr_step = get_number(line);
            }
            idx++;
        }
        if(curr_result == "sat") {
